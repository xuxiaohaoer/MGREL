...
def RandomForest(argv):
    ...
    if n_iter != 0:
        ...
        rnd_clf = RandomizedSearchCV(rnd, param_distributions=param_dist, n_iter=n_iter, n_jobs=-1)
        ...
        rnd_clf = RandomForestClassifier(criterion='entropy', max_leaf_nodes=280,
                       n_estimators=290, n_jobs= -1) # 最初的模型
    ...
    with open('rf_ill.txt', 'a+',encoding='utf-8') as f:
        list = []
        list.append("acc:{:.4f}\n".format(acc))
        list.append("pre:{:.4f}\n".format(pre))
        list.append("rec:{:.4f}\n".format(rec))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--f", type=str, default="flow_feature", help="select feature")
